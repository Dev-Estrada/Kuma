    ...
    try {
        ...
        if (!res.ok) {
            if (typeof showAlert === 'function')
                showAlert({ title: 'Error', message: data.error || (res.status === 403 ? 'Solo administradores pueden crear copias de seguridad.' : 'No se pudo crear la copia.'), type: 'error' });
            return;
        }
        if (data.ok && data.filename) {
            const where = data.fullPath ? `\n\nUbicación:\n${data.fullPath}` : '';
            if (typeof showAlert === 'function')
                showAlert({ title: 'Copia creada', message: `Se guardó la copia de seguridad: ${data.filename}${where}`, type: 'success' });
        }
        else {
            if (typeof showAlert === 'function')
                showAlert({ title: 'Listo', message: 'Copia de seguridad creada correctamente.', type: 'success' });
        }
    }
    catch (e) {
        if (typeof showAlert === 'function')
            showAlert({ title: 'Error', message: 'Error de conexión al crear la copia de seguridad.', type: 'error' });
    }
}
...
document.getElementById('report-modal-day')?.addEventListener('click', (e) => { if (e.target && e.target.id === 'report-modal-day')
    closeReportModal('report-modal-day'); });
document.getElementById('report-modal-period')?.addEventListener('click', (e) => { if (e.target && e.target.id === 'report-modal-period')
    closeReportModal('report-modal-period'); });
document.getElementById('report-modal-inventory')?.addEventListener('click', (e) => { if (e.target && e.target.id === 'report-modal-inventory')
    closeReportModal('report-modal-inventory'); });
